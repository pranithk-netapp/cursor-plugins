...
async function fetchCodeLenses(uri: vscode.Uri): Promise<vscode.CodeLens[]> {
  for (let attempt = 0; attempt < 40; attempt++) {
    const raw = await vscode.commands.executeCommand<vscode.CodeLens[] | null>(
...
function registerFixtureDocumentSymbols(
  ...
  return vscode.languages.registerDocumentSymbolProvider(
    { language: "go", scheme: "file" },
    {
      provideDocumentSymbols(document) {
        ...
        const iface = new vscode.DocumentSymbol(
          "SnapshotProvider",
          "",
...
suite("Cursor Go Implements Indicator E2E", () => {
  const fixturePath = path.join(
    extensionRoot(),
    "test-fixtures/workspace/provider.go"
  );
  ...
  test("fixture interface has multiple implementations", async () => {
    ...
    assert.ok(
      text.includes("type SnapshotProvider interface"),
      "interface not found in fixture"
    );
    assert.ok(
      text.includes("OntapProvider") &&
        text.includes("MockProvider") &&
        text.includes("SdsProvider"),
      "expected implementers missing in fixture"
    );
  });
  ...
  test("code lens click opens peek and captures screenshot (macOS)", async function () {
      ...
      { language: "go", scheme: "file" },
      {
        provideImplementation(document, position) {
          ...
          const implementationLineIndices = document
            ...
            .filter(({ line }) =>
              line.includes("func (") && line.includes("CreateSnapshot(")
            )
            .map(({ index }) => index);
          return implementationLineIndices.map(line => {
            ...
            const start = new vscode.Position(
              line,
              Math.max(0, lineText.indexOf("CreateSnapshot"))
            );
    ...
    try {
      ...
      const methodPosition = new vscode.Position(
        methodLine,
        methodNameColumn + 1
      );
      ...
      assert.ok(
        probeLocations.length >= 3,
        "implementation provider probe did not return expected results"
      );

      const lenses = await fetchCodeLenses(fixtureUri);
      const implLens = lenses.find(
        l => l.command?.command === "cursorGoImplements.show"
      );
      assert.ok(
        implLens?.command,
        `expected implementations CodeLens; got count=${lenses.length} titles=${lenses
      ...
      await vscode.commands.executeCommand(
        implLens.command.command,
        ...(implLens.command.arguments ?? [])
      );
      await sleep(900);
      const editorsAfterPeek = vscode.window.visibleTextEditors.length;
      assert.ok(
        editorsAfterPeek > editorsBeforePeek,
        `peek should add a visible embedded editor (${editorsBeforePeek} -> ${editorsAfterPeek})`
      );

      const outFile = screenshotPath("implementations-peek.png");
      const readmePath = path.join(
        path.dirname(outFile),
        "README-SCREENSHOT.txt"
      );
      try {
      ...
      if (screenshotOk) {
        fs.writeFileSync(
          readmePath,
          "implementations-peek.png was captured via screencapture after opening the implementations peek.\n",
          "utf8"
        );
      } else {
        fs.writeFileSync(
